...
export default function OzbekDetails() {
    const [posts, setPosts] = useState([]);
    const { id } = useParams()

    useEffect(() => {
        axios.get(`http://localhost:9000/ozbek/${id}`)
            .then(res => {
                setPosts(res.data)
            })
    ...
    return (
        <div className="new container">
            <div className="row">
                <div className="col">
                    <Link to="/forozbek"><button className="btn btn colButton"><img className="imgVector" src={Image5} alt="" />Orqaga qaytish</button></Link>
